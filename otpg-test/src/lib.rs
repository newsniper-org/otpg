...
mod tests {
    ...
    use otpg_core::constants::XCHACHA20_NONCE_BYTES;
    ...
    fn test_happy_path_roundtrip() {
        ...
        // --- 1. 설정: 앨리스와 밥의 키 생성 ---
        let (alice_pub, alice_vault) = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::generate_keys::<ThreadRng>(10, &mut rng).unwrap();
        let (_bob_pub, bob_vault) = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::generate_keys::<ThreadRng>(10, &mut rng).unwrap();
        ...
        // --- 2. 실행: 밥이 앨리스에게 암호화 ---
        let ciphertext_bundle = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::encrypt(
            &bob_keys,
            &alice_pub,
            original_plaintext,
        ).unwrap();
        ...
        let decryption_result = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::decrypt(
    ...
    fn test_decrypt_with_wrong_otp() {
        ...
        let (_bob_pub, bob_vault) = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::generate_keys::<ThreadRng>(10, &mut rng).unwrap();
        let bob_keys = unlock_private_keys_for_test(&bob_vault).unwrap();
        let ciphertext_bundle = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::encrypt(&bob_keys, &alice_pub, b"test").unwrap();
    ...
    fn test_decrypt_with_wrong_keys() {
        let mut rng = rand::rng();
        // --- 1. 설정: 앨리스, 밥, 찰리 키 생성 ---
        let (alice_pub, _alice_vault) = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::generate_keys::<ThreadRng>(10, &mut rng).unwrap();
        let (_bob_pub, bob_vault) = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::generate_keys::<ThreadRng>(10, &mut rng).unwrap();
        let bob_keys = unlock_private_keys_for_test(&bob_vault).unwrap();
        let (_charlie_pub, charlie_vault) = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::generate_keys::<ThreadRng>(10, &mut rng).unwrap(); // 찰리의 키
        
        // 밥이 앨리스에게 메시지 암호화
        let bundle_for_alice = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::encrypt(&bob_keys, &alice_pub, b"test").unwrap();
    ...
    fn test_decrypt_with_tampered_ciphertext() {
        ...
        let (_bob_pub, bob_vault) = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::generate_keys::<ThreadRng>(10, &mut rng).unwrap();
        let bob_keys = unlock_private_keys_for_test(&bob_vault).unwrap();
        let mut bundle = otpg_contrib::TotpXChaCha20Poly1305Kyber1024X448BLAKE3Ed448::encrypt(&bob_keys, &alice_pub, b"test").unwrap();
