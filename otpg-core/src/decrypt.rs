...
mod for_creusot {
    ...
    pub const fn able_to_try_decrypt<const KEY_BYTES: usize, const NONCE_BYTES: usize, V: OtpVerifier, C: AeadCipher<KEY_BYTES,NONCE_BYTES> + const HasNonceLength<NONCE_BYTES>, const PQ_CT_BYTES: usize, const KA_PUBKEY_BYTES: usize>(
        ...
        enough && V::verify_spec(otp_code, s_otp, current_timestamp)
    }

    #[requires(able_to_try_decrypt::<DERIVED_KEY_BYTES, NONCE_BYTES, V, C, PQ_CT_BYTES, KA_PUBKEY_BYTES>(bundle, otp_code, recipient_vault.authentication.s_otp.0@, current_timestamp))] // 전제 조건 1: OTP 코드가 유효해야 한다, 전제 조건 2: 암호문 길이가 Nonce 길이보다 길어야 한다.
    #[ensures(result == decrypt_spec::<V, NONCE_BYTES, C, PQ_PUBKEY_BYTES, PQ_PRVKEY_BYTES, PQ_SEC_BYTES, PQ_CT_BYTES, PQ, KA_PUBKEY_BYTES, KA_PRVKEY_BYTES, KA_SEC_BYTES, KA, DERIVED_KEY_BYTES, KD, SIGKEY_BYTES, SIGN_BYTES>(recipient_vault, otp_code, bundle, current_timestamp))] // 결과 보장: 복호화는 성공적으로 완료된다.
    ...
    pub fn decrypt<V: OtpVerifier, const NONCE_BYTES: usize, C: AeadCipher<DERIVED_KEY_BYTES, NONCE_BYTES> + const HasNonceLength<NONCE_BYTES>, const PQ_PUBKEY_BYTES: usize, const PQ_PRVKEY_BYTES: usize, const PQ_SEC_BYTES: usize, const PQ_CT_BYTES: usize, PQ: PostQuantumKEM<PQ_PUBKEY_BYTES, PQ_PRVKEY_BYTES, PQ_SEC_BYTES, PQ_CT_BYTES>, const KA_PUBKEY_BYTES: usize, const KA_PRVKEY_BYTES: usize, const KA_SEC_BYTES: usize, KA: KeyAgreement<KA_PUBKEY_BYTES, KA_PRVKEY_BYTES, KA_SEC_BYTES>, const DERIVED_KEY_BYTES: usize, KD: KDF<DERIVED_KEY_BYTES>, const SIGKEY_BYTES: usize, const SIGN_BYTES: usize>(
        ...
        let (decrypted, _) = decrypt_to_verify::<V, NONCE_BYTES, C, PQ_PUBKEY_BYTES, PQ_PRVKEY_BYTES, PQ_SEC_BYTES, PQ_CT_BYTES, PQ, KA_PUBKEY_BYTES, KA_PRVKEY_BYTES, KA_SEC_BYTES, KA, DERIVED_KEY_BYTES, KD, SIGKEY_BYTES, SIGN_BYTES>(
            recipient_vault,
            otp_code,
            bundle,
            current_timestamp
    ...
    /// 수신자의 개인키 저장소와 OTP 코드, 그리고 암호화된 메시지 묶음을 사용하여 원본 메시지를 복호화합니다.
    pub(crate) fn decrypt_to_verify<V: OtpVerifier, const NONCE_BYTES: usize, C: AeadCipher<DERIVED_KEY_BYTES, NONCE_BYTES> + const HasNonceLength<NONCE_BYTES>, const PQ_PUBKEY_BYTES: usize, const PQ_PRVKEY_BYTES: usize, const PQ_SEC_BYTES: usize, const PQ_CT_BYTES: usize, PQ: PostQuantumKEM<PQ_PUBKEY_BYTES, PQ_PRVKEY_BYTES, PQ_SEC_BYTES, PQ_CT_BYTES>, const KA_PUBKEY_BYTES: usize, const KA_PRVKEY_BYTES: usize, const KA_SEC_BYTES: usize, KA: KeyAgreement<KA_PUBKEY_BYTES, KA_PRVKEY_BYTES, KA_SEC_BYTES>, const DERIVED_KEY_BYTES: usize, KD: KDF<DERIVED_KEY_BYTES>, const SIGKEY_BYTES: usize, const SIGN_BYTES: usize>(
        recipient_vault: &PrivateKeyVault<NONCE_BYTES>,
        otp_code: &str, // 사용자가 입력한 6자리 OTP 코드
        bundle: &CiphertextBundle<KA_PUBKEY_BYTES, PQ_CT_BYTES, NONCE_BYTES>,
        current_timestamp: u64
        ...
        let associated_data = bytes_concat![bundle.sender_identity_key.0,
            recipient_ik_pub_bytes.0];
        let plaintext = C::decrypt_aead(&session_key, &bundle.aead_ciphertext, &associated_data)?;

        // --- 5단계: 평문(Plaintext) 반환 ---
        Ok((plaintext, master_secret))
    }

    #[logic(opaque)]
    pub fn decrypt_spec<V: OtpVerifier, const NONCE_BYTES: usize, C: AeadCipher<DERIVED_KEY_BYTES, NONCE_BYTES> + const HasNonceLength<NONCE_BYTES>, const PQ_PUBKEY_BYTES: usize, const PQ_PRVKEY_BYTES: usize, const PQ_SEC_BYTES: usize, const PQ_CT_BYTES: usize, PQ: PostQuantumKEM<PQ_PUBKEY_BYTES, PQ_PRVKEY_BYTES, PQ_SEC_BYTES, PQ_CT_BYTES>, const KA_PUBKEY_BYTES: usize, const KA_PRVKEY_BYTES: usize, const KA_SEC_BYTES: usize, KA: KeyAgreement<KA_PUBKEY_BYTES, KA_PRVKEY_BYTES, KA_SEC_BYTES>, const DERIVED_KEY_BYTES: usize, KD: KDF<DERIVED_KEY_BYTES>, const SIGKEY_BYTES: usize, const SIGN_BYTES: usize>(
        _recipient_vault: &PrivateKeyVault<NONCE_BYTES>,
        _otp_code: &str, // 사용자가 입력한 6자리 OTP 코드
        _bundle: &CiphertextBundle<KA_PUBKEY_BYTES, PQ_CT_BYTES, NONCE_BYTES>,
...
#[cfg(creusot)]
pub(crate) use for_creusot::decrypt;
#[cfg(creusot)]
pub(crate) use for_creusot::decrypt_to_verify;
#[cfg(creusot)]
pub use for_creusot::decrypt_spec;
#[cfg(not(creusot))]
pub(crate) use not_for_creusot::decrypt;