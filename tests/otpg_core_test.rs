// tests/otpg_core_test.rs

use chacha20poly1305::aead::Aead;
use chacha20poly1305::KeyInit;
use otpg::error::OtpgError;
use otpg::keygen;
use otpg::encrypt;
use otpg::decrypt;
use totp_rs::Rfc6238;
...
fn unlock_private_keys_for_test(
    vault: &otpg::types::PrivateKeyVault,
) -> otpg::error::Result<otpg::types::PrivateKeyBundle> {
    // 1. KEK(키 암호화 키) 재유도
    ...
    ).map_err(|_| otpg::error::OtpgError::AeadError)?;

    // 3. 개인키 묶음 역직렬화
    // 실제 decrypt 함수 1.4 단계와 동일합니다.
    let (private_keys, _) = bincode::serde::decode_from_slice::<otpg::types::PrivateKeyBundle, _>(&plaintext_bytes, bincode::config::standard().with_fixed_int_encoding())?;
