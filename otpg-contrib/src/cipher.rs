...
use otpg_core::types::{Bytes, GetContextStr};
...
impl AeadCipher<XCHACHA20_NONCE_LEN> for XChaCha20Poly1305Cipher {
    fn encrypt_aead(key: &[u8], plaintext: &[u8], associated_data: &[u8]) -> Result<(Bytes<XCHACHA20_NONCE_LEN>, Vec<u8>)> {
...
impl KeyAgreement for X448KeyAgreement {
    fn derive_when_encrypt(sender_keys: &otpg_core::types::PrivateKeyBundle, recipient_bundle: &otpg_core::types::PublicKeyBundle) -> Result<(u32, Vec<u8>, Vec<u8>, Vec<u8>)> {
        // --- 1. 사용할 수신자의 일회성 사전 키(OPK) 랜덤 선택 ---
        let mut rng = rand::rng();
        let opks = recipient_bundle.one_time_prekeys.clone();
        let (&opk_id, recipient_opk_pub) = opks.iter().choose(&mut rng).ok_or(OtpgError::NoPreKeyAvailable).unwrap();
        ...
        // 발신자 키
        let sender_ik_pkey = PKey::private_key_from_raw_bytes(&sender_keys.identity_key_kx, Id::X448).unwrap();

        // 수신자 키
        let recipient_ik_pkey = PKey::public_key_from_raw_bytes(&recipient_bundle.identity_key, Id::X448).unwrap();
        let recipient_spk_pkey = PKey::public_key_from_raw_bytes(&recipient_bundle.signed_prekey.key, Id::X448).unwrap();
        let recipient_opk_pkey = PKey::public_key_from_raw_bytes(recipient_opk_pub, Id::X448).unwrap();
        ...
        Ok((
            opk_id, 
            classic_dh_secrets, 
            sender_ik_pkey.raw_public_key()?, 
            sender_ephemeral_key.raw_public_key()?
        ))
    }
    ...
    fn derive_when_decrypt(recipient_keys: &otpg_core::types::PrivateKeyBundle, bundle: &otpg_core::types::CiphertextBundle, shared_secret_pq: &[u8]) -> (Vec<u8>, Vec<u8>) {
        ...
        let sender_ik_pkey = PKey::public_key_from_raw_bytes(&bundle.sender_identity_key, Id::X448).unwrap();
        let sender_ek_pkey = PKey::public_key_from_raw_bytes(&bundle.sender_ephemeral_key, Id::X448).unwrap();
        
        let recipient_ik_pkey = PKey::private_key_from_raw_bytes(&recipient_keys.identity_key_kx, Id::X448).unwrap();
        let recipient_spk_pkey = PKey::private_key_from_raw_bytes(&recipient_keys.signed_prekey, Id::X448).unwrap();
        ...
        let recipient_opk_pkey = PKey::private_key_from_raw_bytes(&recipient_opk_bytes, Id::X448).unwrap();
...
impl OneTimePrekeysPairGen for X448KeyAgreement {
    fn gen_opkspair(num_opks: u32) -> (HashMap<u32, Vec<u8>>, HashMap<u32, Vec<u8>>) {
        let opks: HashMap<u32, openssl::pkey::PKey<openssl::pkey::Private>> = (0..num_opks)
            .map(|id| (id, openssl::pkey::PKey::generate_x448().unwrap()))
            .collect();
        let opks_pub = opks
            .iter()
            .map(|(id, sk)| {
                (*id, sk.raw_public_key().unwrap())
            })
            .collect();
        let opks_prv = opks
            .iter()
            .map(|(id, sk)| {
                (*id, sk.raw_private_key().unwrap())
            })
            .collect();
        (opks_pub, opks_prv)
    }
...
impl Signer for ED448Signer {
    fn sign<R: rand::CryptoRng + ?Sized>(msg: &[u8], rng: &mut R) -> (Vec<u8>, Vec<u8>) {
        ...
        (ik_sig.to_bytes().to_vec(), signature.to_bytes().to_vec())
    }
}